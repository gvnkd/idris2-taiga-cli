...
wrapResult encodeFn (Left err) =
  Err $ MkErrorResponse False "api_error" err
wrapResult encodeFn (Right val) =
  Ok $ MkSuccess True (encodeFn val)

dispatchPing' : HasIO io => Maybe String -> io Response
dispatchPing' Nothing =
  pure $ Err $ MkErrorResponse False "no_base" "No base URL provided"
dispatchPing' (Just baseUrl) = do
  let pingUrl : _ = baseUrl ++ "/api/v1/me"
  resp <- Taiga.Api.httpGet pingUrl Nothing
  let status_code : _ = resp.status.code
  if status_code >= 200 && status_code < 400
    then pure $ Ok $ MkSuccess True "ok"
      else
        if status_code == 0
          then pure $ Err $ MkErrorResponse False "network_error" "Failed to connect to server"
            else
              pure $ Err $ MkErrorResponse False "server_error" $ "Server responded with status " ++ show status_code

dispatchLogin' : HasIO io => Credentials -> Maybe String -> io Response
dispatchLogin' _ Nothing =
  pure $ Err $ MkErrorResponse False "no_base" "No base URL provided"
dispatchLogin' creds (Just baseUrl) =
  map (wrapResult encode) (login baseUrl creds)

dispatchRefresh' : HasIO io => String -> Maybe String -> io Response
dispatchRefresh' _ Nothing =
  pure $ Err $ MkErrorResponse False "no_base" "No base URL provided"
dispatchRefresh' refreshTok (Just baseUrl) =
  map (wrapResult encode) (refreshToken baseUrl refreshTok)

dispatchWithEnvHelper : HasIO io => io (Either String a) -> (a -> String) -> io Response
dispatchWithEnvHelper action encFn =
  map (wrapResult encFn) action

runList : {a : Type} -> HasIO io => io (Either String (List a, PaginationMeta)) -> io (Either String (List a))
runList =
  map (map fst)

dispatchWithEnv' : HasIO io => (command : Command) -> (env : ApiEnv) -> io Response
dispatchWithEnv' command env =
  case command of
    ...
    CmdListTasks args => dispatchWithEnvHelper (runList $ (listTasks @{env}) args.project Nothing args.status args.page args.pageSize) encode
...
public export
dispatchCommand : HasIO io => (command : Command) -> (auth : Maybe Model.Auth.Token) -> (base : Maybe String) -> io Response
dispatchCommand (CmdLogin creds) _ base =
  dispatchLogin' creds base
dispatchCommand CmdPing _ base =
  dispatchPing' base
dispatchCommand (CmdRefresh rtok) _ base =
  dispatchRefresh' rtok base
dispatchCommand command Nothing _ =
  pure $ Err $ MkErrorResponse False "unauthorized" "No token provided"
dispatchCommand _ _ Nothing =
  pure $ Err $ MkErrorResponse False "no_base" "No base URL provided"
dispatchCommand command (Just token) (Just baseUrl) =
  ...
  where
    env : ApiEnv
    env =
      MkApiEnv baseUrl token.auth_token
