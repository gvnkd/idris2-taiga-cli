...
public export
getProjectEnvFull : AppM (ApiEnv, (Nat64Id, Maybe Project))
getProjectEnvFull = do
  st <- liftIOEither loadState
  pid <- liftEither $ getActiveProject st
  env <- resolveApiEnv
  pure (env, (pid, st.project_cache))

public export
getCachedProject : AppM (Maybe Project)
getCachedProject = do
  st <- liftIOEither loadState
  pure st.project_cache
...
resolveRef ref = do
  ...
  case (decodeEither {a = ResolveResponse}) jsonStr of
    ...
    Right res => case extractEntityFromResolve res of
...
resolveToIdForType expectedKind s = do
  ...
  if entityType == resolverKey expectedKind
    then pure nid
        ...
        appFail $ "Identifier " ++ s ++ " resolved to wrong entity type"

public export
resolveTaskId : String -> AppM Nat64Id
resolveTaskId =
  resolveToIdForType TaskK

public export
resolveEpicId : String -> AppM Nat64Id
resolveEpicId =
  resolveToIdForType EpicK

public export
resolveStoryId : String -> AppM Nat64Id
resolveStoryId =
  resolveToIdForType StoryK

public export
resolveIssueId : String -> AppM Nat64Id
resolveIssueId =
  resolveToIdForType IssueK

public export
resolveWikiId : String -> AppM Nat64Id
resolveWikiId =
  resolveToIdForType WikiK

public export
resolveMilestoneId : String -> AppM Nat64Id
resolveMilestoneId =
  resolveToIdForType MilestoneK

public export
getProjectForStatus : ApiEnv -> AppSt -> AppM Project
getProjectForStatus env st =
  case st.project_cache of
    Just proj => pure proj
    Nothing => case st.active_project of
                 Nothing => appFail "No active project set"
                 Just pid => liftIOEither $ (getProjectById @{env}) pid

public export
resolveStatus : ApiEnv -> AppSt -> String -> String -> AppM Bits64
...
public export
resolveOptionalStatus : ApiEnv -> AppSt -> String -> Maybe String -> AppM (Maybe Bits64)
...
public export
callToResult : ToJSON a => (a -> String) -> AppM a -> AppM CmdResult
callToResult fmt action =
  map (\val => cmdOk (fmt val) val) action `catchE` (\err => pure (cmdError err))

public export
handleEntityList : ToJSON a => String -> (Maybe Project -> List a -> String) -> (ApiEnv -> String -> IO (Either String (List a, PaginationMeta))) -> IO (Either String CmdResult)
handleEntityList name fmt listFn =
  runAppM $ do
...
public export
handleEntityGet : ToJSON a => String -> (Maybe Project -> a -> String) -> (String -> AppM Nat64Id) -> (ApiEnv -> Nat64Id -> IO (Either String a)) -> String -> IO (Either String CmdResult)
handleEntityGet name fmt resolveId getFn ident =
  runAppM $ do
              eid <- resolveId ident
              env <- resolveApiEnv
              st <- liftIOEither loadState
              val <- liftIOEither $ getFn env eid
              pure $ cmdOk (fmt st.project_cache val) val

public export
handleEntityDelete : String -> (String -> AppM Nat64Id) -> (ApiEnv -> Nat64Id -> IO (Either String ())) -> String -> IO (Either String CmdResult)
handleEntityDelete name resolveId deleteFn ident =
  runAppM $ do
              eid <- resolveId ident
              env <- resolveApiEnv
              ...
              if not confirmed
                then pure $ cmdInfo "Delete cancelled"
                    ...
                    do
                      ...
                      let dr : _ = MkDeleteResult name eid.id
...
initAux maybeBaseUrl = do
  let baseUrl : _ = case maybeBaseUrl of
                      Just u => u
...
handleInit maybeBaseUrl =
  runAppM (initAux maybeBaseUrl)
...
loginIO user mpass =
  do
    ...
    let creds : _ = MkCredentials user password
    st_e <- loadState
    case st_e of
  ...
  where
    ...
    getPassword Nothing =
      readPassword
handleLogin user mpass =
  runAppM $ liftIOEither $ loginIO user mpass
  ...
  where
    ...
    formatShow st =
      let projId = case st.active_project of
                     Nothing => "(none)"
                     Just p => show p.id in "Base URL: " ++ st.base_url ++ "\nActive project: " ++ projId
handleShow =
  runAppM showAux
...
projectAccessError ident =
  unlines ["Cannot access project '" ++ ident ++ "' by slug.", "", "The project exists in your project list, but the slug-based lookup", "endpoint requires full project membership. You may have only", "public/view-level access.", "", "Try using the numeric project ID instead:", "  tcli project set <number>", "", "To find the numeric ID, run:", "  tcli project list"]
...
projectSetAux ident = do
  ...
  pure res
handleProjectSet ident =
  runAppM (projectSetAux ident)
...
handleTaskList maybeStatus =
  runAppM (taskListAux maybeStatus)
...
handleTaskCreate subject =
  runAppM (taskCreateAux subject)
...
taskGetAux ident = do
  ...
  pure $ cmdOk (formatTask mProj val) val
handleTaskGet ident =
  runAppM (taskGetAux ident)
...
handleTaskStatus ident statusId =
  runAppM (taskStatusAux ident statusId)
...
handleTaskComment ident text =
  runAppM (taskCommentAux ident text)
...
handleTaskAssignStory taskIdent storyIdent =
  runAppM (taskAssignStoryAux taskIdent storyIdent)

resolveDescription : Maybe String -> AppM (Maybe String)
resolveDescription Nothing =
  pure Nothing
resolveDescription (Just s) = do
  case unpack s of
    ('@' :: rest) => do
                       let path : _ = pack rest
...
resolveUpdateStatus : ApiEnv -> String -> Maybe String -> Maybe Bits64 -> AppM (Maybe Bits64)
resolveUpdateStatus env entityType mStatusText mStatusId =
  case mStatusId of
    Just id => pure $ Just id
    Nothing => traverse (\txt => do
                                   state <- liftIOEither loadState
                                   resolveStatus env state entityType txt) mStatusText
...
taskUpdateAux ident mSubject mDesc mStatusText mStatusId = do
  ...
  let subj : _ = fromMaybe current.subject mSubject
  let desc : _ = fromMaybe current.description resolved_desc
...
handleTaskUpdate ident mSubject mDesc mStatusText mStatusId =
  runAppM (taskUpdateAux ident mSubject mDesc mStatusText mStatusId)

public export
handleTaskDelete : String -> IO (Either String CmdResult)
handleTaskDelete =
  handleEntityDelete "task" resolveTaskId (\e => \i => (deleteTask @{e}) i)

public export
handleEpicDelete : String -> IO (Either String CmdResult)
handleEpicDelete =
  handleEntityDelete "epic" resolveEpicId (\e => \i => (deleteEpic @{e}) i)
...
handleEpicUpdate ident mSubject mDesc mStatusText mStatusId =
  runAppM (epicUpdateAux ident mSubject mDesc mStatusText mStatusId)

public export
handleEpicList : IO (Either String CmdResult)
handleEpicList =
  handleEntityList "Epic" formatEpicSummaries (\e => \p => (listEpics @{e}) (Just p) Nothing (Just 1000))

public export
handleEpicGet : String -> IO (Either String CmdResult)
handleEpicGet =
  handleEntityGet "Epic" formatEpic resolveEpicId (\e => \i => (getEpic @{e}) i)
...
handleEpicCreate subject mDesc mStatus =
  runAppM (epicCreateAux subject mDesc mStatus)

public export
handleSprintList : IO (Either String CmdResult)
handleSprintList =
  handleEntityList "Sprint" formatMilestoneSummaries (\e => \p => (listMilestones @{e}) (Just p) Nothing (Just 1000))
...
handleSprintSet =
  handleEntityGet "Sprint" (\_ => \m => formatMilestone m) resolveMilestoneId (\e => \i => (getMilestone @{e}) i)

public export
handleIssueList : IO (Either String CmdResult)
handleIssueList =
  handleEntityList "Issue" formatIssueSummaries (\e => \p => (listIssues @{e}) (Just p) Nothing (Just 1000))

public export
handleIssueGet : String -> IO (Either String CmdResult)
handleIssueGet =
  handleEntityGet "Issue" formatIssue resolveIssueId (\e => \i => (getIssue @{e}) i)
...
handleIssueCreate subject mDesc mPriority mSeverity mType =
  runAppM (issueCreateAux subject mDesc mPriority mSeverity mType)
...
handleIssueUpdate ident mSubject mDesc mType mStatusText mStatusId =
  runAppM (issueUpdateAux ident mSubject mDesc mType mStatusText mStatusId)

public export
handleIssueDelete : String -> IO (Either String CmdResult)
handleIssueDelete =
  handleEntityDelete "issue" resolveIssueId (\e => \i => (deleteIssue @{e}) i)

public export
handleStoryList : IO (Either String CmdResult)
handleStoryList =
  handleEntityList "Story" formatStorySummaries (\e => \p => (listStories @{e}) (Just p) Nothing (Just 1000))

public export
handleStoryGet : String -> IO (Either String CmdResult)
handleStoryGet =
  handleEntityGet "Story" formatStory resolveStoryId (\e => \i => (getStory @{e}) i)

public export
handleWikiList : IO (Either String CmdResult)
handleWikiList =
  handleEntityList "Wiki page" formatWikiPageSummaries (\e => \p => (listWiki @{e}) (Just p) Nothing (Just 1000))

public export
handleWikiGet : String -> IO (Either String CmdResult)
handleWikiGet =
  handleEntityGet "Wiki page" (\_ => \w => formatWikiPage w) resolveWikiId (\e => \i => (getWiki @{e}) i)
...
handleWikiCreate slug content =
  runAppM (wikiCreateAux slug content)
...
storyCreateAux subject mDesc mMilestone = do
  resolved_desc <- resolveDescription mDesc
  (env, (pid, mProj)) <- getProjectEnvFull
  let mMs : _ = map MkNat64Id $ mMilestone >>= readNat
  val <- liftIOEither $ (createStory @{env}) (show pid.id) subject resolved_desc mMs
  pure $ cmdOk ("Story created\n" ++ formatStory mProj val) val
handleStoryCreate subject mDesc mMilestone =
  runAppM (storyCreateAux subject mDesc mMilestone)
...
storyUpdateAux ident mSubject mDesc mMilestone mStatusText mStatusId = do
  ...
  let subj : _ = fromMaybe current.subject mSubject
  let desc : _ = fromMaybe current.description resolved_desc
  let mMs : _ = fromMaybe Nothing (map Just mMilestone)
...
handleStoryUpdate ident mSubject mDesc mMilestone mStatusText mStatusId =
  runAppM (storyUpdateAux ident mSubject mDesc mMilestone mStatusText mStatusId)

public export
handleStoryDelete : String -> IO (Either String CmdResult)
handleStoryDelete =
  handleEntityDelete "story" resolveStoryId (\e => \i => (deleteStory @{e}) i)
...
wikiUpdateAux ident mContent mSlug = do
  ...
  let content : _ = case mContent of
                      Nothing => current.content
                      Just c => c
  let slug : _ = case mSlug of
...
handleWikiUpdate ident mContent mSlug =
  runAppM (wikiUpdateAux ident mContent mSlug)

public export
handleWikiDelete : String -> IO (Either String CmdResult)
handleWikiDelete =
  handleEntityDelete "wiki page" resolveWikiId (\e => \i => (deleteWiki @{e}) i)
...
sprintCreateAux name mStart mEnd = do
  case (mStart, mEnd) of
    (Nothing, _) => appFail "sprint create requires --start DATE and --end DATE"
    (_, Nothing) => appFail "sprint create requires --start DATE and --end DATE"
...
handleSprintCreate name mStart mEnd =
  runAppM (sprintCreateAux name mStart mEnd)
...
handleSprintUpdate ident mName mStart mEnd ver =
  runAppM (sprintUpdateAux ident mName mStart mEnd ver)

public export
handleSprintDelete : String -> IO (Either String CmdResult)
handleSprintDelete =
  handleEntityDelete "sprint" resolveMilestoneId (\e => \i => (deleteMilestone @{e}) i)

fetchEntityVersion : ApiEnv -> EntityKind -> Nat64Id -> IO (Either String Bits32)
...
commentAddAux entityName ident text = do
  case parseEntityKind entityName of
    Nothing => appFail $ "Unknown entity type: " ++ entityName ++ ". Use: task, issue, story, wiki"
    Just kind => do
                   eid <- resolveToIdForType kind ident
                   env <- resolveApiEnv
                   ver <- liftIOEither $ fetchEntityVersion env kind eid
                   raw <- liftIOEither $ (addComment @{env}) (apiEntityName kind) eid text ver
                   pure $ cmdOkRaw "Comment added" raw
handleCommentAdd entityName ident text =
  runAppM (commentAddAux entityName ident text)
...
commentListAux entityName ident = do
  case parseEntityKind entityName of
    ...
    Just kind => do
                   eid <- resolveToIdForType kind ident
                   env <- resolveApiEnv
                   val <- liftIOEither $ (listHistory @{env}) (apiEntityName kind) eid
                   pure $ cmdOk (formatHistoryEntries val) val
handleCommentList entityName ident =
  runAppM (commentListAux entityName ident)
...
statusListAux entityType = do
  case parseEntityKind entityType of
    ...
    Just kind => do
                   ...
                   env <- resolveApiEnv
                   proj <- getProjectForStatus env st
                   let statuses : _ = statusesOf proj kind
                   let title : _ = entityTypeName kind ++ " statuses"
                   pure $ cmdInfo (title ++ ":\n" ++ formatStatusList statuses)
handleStatusList entityType =
  runAppM (statusListAux entityType)

firstSuccessNamed : List (String, AppM String) -> AppM (String, String)
firstSuccessNamed [] =
  appFail "All entity getters failed"
firstSuccessNamed ((name, m) :: rest) =
  catchE (do
            json <- m
            pure (name, json)) (\_ => firstSuccessNamed rest)

public export
handleResolve : String -> IO (Either String CmdResult)
handleResolve ref =
  runAppM (resolveAndLookup ref)
  where
    resolveAndLookup : String -> AppM CmdResult
    resolveAndLookup ref = do
      (_, nid) <- resolveRef ref
      env <- resolveApiEnv
      let getters : List (String, AppM String) = [("task", liftIOEither $ map (map encode) $ (getTask @{env}) nid), ("issue", liftIOEither $ map (map encode) $ (getIssue @{env}) nid), ("story", liftIOEither $ map (map encode) $ (getStory @{env}) nid), ("epic", liftIOEither $ map (map encode) $ (getEpic @{env}) nid), ("wiki", liftIOEither $ map (map encode) $ (getWiki @{env}) nid), ("sprint", liftIOEither $ map (map encode) $ (getMilestone @{env}) nid)]
      catchE (do
                (name, json) <- firstSuccessNamed getters
                pure $ cmdOkRaw ("Resolved ref " ++ ref ++ " to " ++ name ++ " id=" ++ show nid.id) json) (\_ => pure $ cmdOk ("Resolved ref " ++ ref ++ " to id " ++ show nid.id) nid)

public export
executeAction : Action -> IO (Either String CmdResult)
executeAction (ActInit mbase) =
  handleInit mbase
executeAction (ActLogin user mpass) =
  handleLogin user mpass
executeAction ActLogout =
  handleLogout
executeAction ActShow =
  handleShow
executeAction ActProjectList =
  handleProjectList
executeAction (ActProjectSet slug) =
  handleProjectSet slug
executeAction ActProjectGet =
  handleProjectGet
executeAction (ActTaskList mstatus) =
  handleTaskList mstatus
executeAction (ActTaskCreate subj) =
  handleTaskCreate subj
executeAction (ActTaskGet tid) =
  handleTaskGet tid
executeAction (ActTaskUpdate tid subj desc stext sid) =
  handleTaskUpdate tid subj desc stext sid
executeAction (ActTaskDelete tid) =
  handleTaskDelete tid
executeAction (ActTaskStatus tid st) =
  handleTaskStatus tid st
executeAction (ActTaskComment tid txt) =
  handleTaskComment tid txt
executeAction (ActTaskAssignStory taskId storyId) =
  handleTaskAssignStory taskId storyId
executeAction ActEpicList =
  handleEpicList
executeAction (ActEpicGet eid) =
  handleEpicGet eid
executeAction (ActEpicCreate subj d s) =
  handleEpicCreate subj d s
executeAction (ActEpicUpdate eid subj d st sid) =
  handleEpicUpdate eid subj d st sid
executeAction (ActEpicDelete eid) =
  handleEpicDelete eid
executeAction ActSprintList =
  handleSprintList
executeAction ActSprintShow =
  handleSprintShow
executeAction (ActSprintSet sid) =
  handleSprintSet sid
executeAction (ActSprintCreate name start end) =
  handleSprintCreate name start end
executeAction (ActSprintUpdate sid name start end ver) =
  handleSprintUpdate sid name start end ver
executeAction (ActSprintDelete sid) =
  handleSprintDelete sid
executeAction ActIssueList =
  handleIssueList
executeAction (ActIssueGet iid) =
  handleIssueGet iid
executeAction (ActIssueCreate subj d p s t) =
  handleIssueCreate subj d p s t
executeAction (ActIssueUpdate iid subj d t st sid) =
  handleIssueUpdate iid subj d t st sid
executeAction (ActIssueDelete iid) =
  handleIssueDelete iid
executeAction ActStoryList =
  handleStoryList
executeAction (ActStoryGet sid) =
  handleStoryGet sid
executeAction (ActStoryCreate subj d m) =
  handleStoryCreate subj d m
executeAction (ActStoryUpdate sid subj d m st sid2) =
  handleStoryUpdate sid subj d m st sid2
executeAction (ActStoryDelete sid) =
  handleStoryDelete sid
executeAction ActWikiList =
  handleWikiList
executeAction (ActWikiGet wid) =
  handleWikiGet wid
executeAction ActTaskStatuses =
  handleStatusList "task"
executeAction ActIssueStatuses =
  handleStatusList "issue"
executeAction ActStoryStatuses =
  handleStatusList "us"
executeAction ActEpicStatuses =
  handleStatusList "epic"
executeAction (ActWikiCreate slug content) =
  handleWikiCreate slug content
executeAction (ActWikiUpdate wid content slug) =
  handleWikiUpdate wid content slug
executeAction (ActWikiDelete wid) =
  handleWikiDelete wid
executeAction (ActCommentAdd entity ident text) =
  handleCommentAdd entity ident text
executeAction (ActCommentList entity ident) =
  handleCommentList entity ident
executeAction (ActResolve ref) =
  handleResolve ref
