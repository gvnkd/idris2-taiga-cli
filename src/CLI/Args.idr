...
listArgs project =
  MkListArgs project Nothing Nothing Nothing Nothing Nothing ""
...
toCommand (ArgBase _) =
  CmdMe
toCommand (ArgLogin u p) =
  CmdLogin $ MkCredentials u p
toCommand ArgMe =
  CmdMe
toCommand (ArgListProjects m) =
  CmdListProjects m
toCommand (ArgGetProject mid ms) =
  CmdGetProject mid ms
toCommand (ArgListEpics p) =
  CmdListEpics (listArgs (Just p))
toCommand (ArgGetEpic mid) =
  CmdGetEpic mid
toCommand (ArgListStories p) =
  CmdListStories (listArgs (Just p))
toCommand (ArgGetStory mid) =
  CmdGetStory mid
toCommand (ArgListTasks p) =
  CmdListTasks (listArgs p)
toCommand (ArgGetTask mid) =
  CmdGetTask mid
toCommand (ArgListIssues p) =
  CmdListIssues (listArgs (Just p))
toCommand (ArgGetIssue mid) =
  CmdGetIssue mid
toCommand (ArgListWiki p) =
  CmdListWiki (listArgs (Just p))
toCommand (ArgGetWiki mid) =
  CmdGetWiki mid
toCommand (ArgListMilestones p) =
  CmdListMilestones (listArgs (Just p))
toCommand (ArgListUsers p) =
  CmdListUsers p
toCommand (ArgListMemberships p) =
  CmdListMemberships p
toCommand (ArgListRoles p) =
  CmdListRoles p
toCommand (ArgSearch p t) =
  CmdSearch p t
toCommand (ArgResolve p r) =
  CmdResolve p r
toCommand (ArgCreateEpic p s d st) =
  CmdCreateEpic p s d st
toCommand (ArgUpdateEpic id s d st v) =
  CmdUpdateEpic id s d st v
toCommand (ArgDeleteEpic id) =
  CmdDeleteEpic id
toCommand (ArgCreateStory p s d m) =
  CmdCreateStory p s d m
toCommand (ArgUpdateStory id s d m v) =
  CmdUpdateStory id s d m v
toCommand (ArgDeleteStory id) =
  CmdDeleteStory id
toCommand (ArgCreateTask p s st d st2 ms) =
  CmdCreateTask p s st d st2 ms
toCommand (ArgUpdateTask id s d st v) =
  CmdUpdateTask id s d st v
toCommand (ArgDeleteTask id) =
  CmdDeleteTask id
toCommand (ArgCreateIssue p s d pr sv it) =
  CmdCreateIssue p s d pr sv it
toCommand (ArgUpdateIssue id s d it st v) =
  CmdUpdateIssue id s d it st v
toCommand (ArgDeleteIssue id) =
  CmdDeleteIssue id
toCommand (ArgCreateWiki p sl c) =
  CmdCreateWiki p sl c
toCommand (ArgUpdateWiki id c sl v) =
  CmdUpdateWiki id c sl v
toCommand (ArgDeleteWiki id) =
  CmdDeleteWiki id
toCommand (ArgComment e id t) =
  CmdComment e id t
toCommand (ArgListComments e id) =
  CmdListComments e id
toCommand (ArgWatchTask id) =
  CmdWatchTask id
toCommand (ArgChangeTaskStatus id st v) =
  CmdChangeTaskStatus id st v
toCommand (ArgTaskComment id t v) =
  CmdTaskComment id t v
toCommand (ArgCreateMilestone p n es ef) =
  CmdCreateMilestone p n es ef
toCommand (ArgUpdateMilestone id n es ef v) =
  CmdUpdateMilestone id n es ef v
toCommand (ArgDeleteMilestone id) =
  CmdDeleteMilestone id
