...
normalize : Path -> Path
normalize path =
  decompose $ trim_root path.components
  where
    ...
    rid_current components =
      filter (not . is_current) components
    rid_parent : List PathComponent -> List PathComponent -> List PathComponent
    rid_parent acc [] =
      reverse acc
    rid_parent acc@(Parent :: as) (Parent :: xs) =
      rid_parent (Parent :: acc) xs
    rid_parent acc@(Root :: as) (Parent :: xs) =
      rid_parent acc xs
    rid_parent (a :: as) (Parent :: xs) =
      rid_parent as xs
    rid_parent acc (x :: xs) =
      rid_parent (x :: acc) xs
    append_current : List PathComponent -> List1 PathComponent
    append_current (Named x :: xs) =
      Current ::: (Named x :: xs)
    append_current [] =
      Current ::: []
    append_current (x :: xs) =
    ...
    decompose (x ::: xs) =
      MkPath $ append_current $ rid_parent [] (x :: rid_current xs)
    trim_root : List1 PathComponent -> List1 PathComponent
    trim_root list =
      case dropWhile (/= Root) $ forget list of
        [] => list
        (x :: xs) => x ::: xs
...
export
implementation Semigroup Path where
  a <+> b =
    let as = (normalize a).components in let ai = init as in normalize $ MkPath $ concat ai (normalize b).components

export
implementation Show Path where
  show path =
    trim_leading_roots $ join "/" $ map component_to_string path.components
    where
      trim_leading_roots : String -> String
      trim_leading_roots str =
        assert_total $ (if isPrefixOf "//" str then strTail str else str)
      ...
      component_to_string Current =
        "."
      component_to_string (Named x) =
        x

export
implementation FromString Path where
  fromString "" =
    MkPath $ singleton Current
  fromString string =
