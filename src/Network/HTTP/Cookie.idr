...
serialize_cookie_no_attr cookie =
  "\{cookie.key}=\{cookie.value}"

export
serialize_cookie : Cookie -> String
serialize_cookie cookie =
  join "; " (serialize_cookie_no_attr cookie :: map serialize_attr cookie.attributes)
  where
    serialize_attr : (String, Maybe String) -> String
    serialize_attr (k, Nothing) =
      k
    serialize_attr (k, Just v) =
      "\{k}=\{v}"
...
add_cookie jar cookie =
  MkCookieJar $ (cookie :: filter (not . same_key cookie) jar.cookies)

export
deserialize_cookie : String -> Maybe Cookie
deserialize_cookie string =
  do
  ...
  where
    ...
    parse_attr attr =
      bimap ltrim (guard (isInfixOf "=" attr) $>) $ splitBy '=' attr
