...
close_pool cond pool = do
  ...
  -- broadcast kill event; workers will close their own sockets
...
wait_for_worker_close mutex cond pools = do
  ...
  if null (join workers)
    then pure ()
    else do conditionWaitTimeout cond mutex 1000000
            wait_for_worker_close mutex cond pools
